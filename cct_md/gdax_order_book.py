...
from shared_log.logger import logger, logger_ws

'''@TODO: USE AUTH CLIENT'''
from gdax.public_client import PublicClient 
from gdax.websocket_client import WebsocketClient

class OrderBook(WebsocketClient):
    def __init__(self, product_id='BTC-USD', log_msg=False):
...
#             pickle.dump(message, self._log_to)
        if self._log_msg:
            logger_ws.info(message)
        sequence = message['sequence']
        if self._sequence == -1:
            self._asks = RBTree()
            self._bids = RBTree()
            res = self._client.get_product_order_book(product_id=self.products[0], level=3)
            for bid in res['bids']:
                self.add({
                    'id': bid[2],
                    'side': 'buy',
        ...
        if sequence <= self._sequence:
            ...
            logger.error('Messages missing ({} - {}). Re-initializing websocket.'.format(sequence, self._sequence))
            self.close()
            self.start()
            return
    ...
    def on_error(self, e):
        logger.error('Market data error. Re-initializing websocket. Msg = {}'.format(e))
        self._sequence = -1
        self.close()
        self.start()
